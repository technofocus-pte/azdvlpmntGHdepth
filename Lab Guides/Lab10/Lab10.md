---
lab:
  title: 'Lab 10: Fixing a production incident using GitHub Copilot Agent mode'
...
2.  If it still fails, select the test and the relevant API code, then:
    ...
    **Prompt:**
    ...
    ```
    /fix This test expects a 400 status code when 'currency' is missing from the POST /payouts request body. The endpoint should validate required
